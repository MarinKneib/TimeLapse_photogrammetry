...
resultsdir = 'V:/gl_kneibm/Projects/PI/2024_CAIRN-GLOBAL/TL_photogrammetry_Argentiere/results/Point_clouds_LB/PhotoTL/full_period/Photo_time-lapse_export_metashape/'
...
for ii in [1883]:
    ...
    # --- Select chunk by name ---
    target_chunk_name = "Part1_PGCP_Optimize_k1k2k3_adaptive_Part2"

    chunk = None
    for ch in doc.chunks:
        if ch.label == target_chunk_name:
            chunk = ch
            break

    if chunk is None:
        raise Exception(f"Chunk '{target_chunk_name}' not found in {path}")
    ...
    # build dense cloud
    chunk.buildDepthMaps(downscale=4, filter_mode=Metashape.MildFiltering) # downscale (int) – Depth map quality (1 - Ultra high, 2 - High, 4 - Medium, 8 - Low, 16 - Lowest).
